...
### 3.2 펌웨어 업그레이드 — 누적 단계별 구현

> A안/B안 같은 **선택지가 아니라**, 이전 단계 위에 쌓이는 **누적 구조**입니다.
> 단계 3을 구현해도 단계 1/2의 기능은 그대로 유지됩니다.
...
#### 단계 3: SPI 고속 OTA 추가 (성능 최적화 — 선택)

> 단계 2 위에 추가. S3 OTA 전송을 UART 대신 SPI(30MHz)로 전환하여 속도 개선.

```
[DeepCoConnector / PC 브라우저]
  │
  └─ S3 펌웨어:  PC → Wi-Fi → http://192.168.4.1/ota_s3 (RTL 경유)
                 RTL이 HTTP로 .bin 수신 → SPI로 S3에 고속 전달
                 S3가 SPI OTA 타입 블록 수신 → Update.h로 기록 → 재부팅
```
...
| 구현 항목 | 위치 | 난이도 | 설명 |
|-----------|------|--------|------|
| SPI OTA 타입 추가 | S3 + RTL | 중간 | `DCM2` 헤더의 type에 `0x20 = OTA_DATA` 추가 |
| SPI OTA 수신기 | S3 | 중간 | SPI 블록 중 OTA 타입 감지 → `Update.h`로 Flash 기록 |
| RTL OTA→SPI 전환 | RTL | 중간 | `/ota_s3` 엔드포인트에서 UART 대신 SPI Master로 전달 |

**전송 속도 비교:**

| 펌웨어 크기 | 단계 2 (UART 115200) | 단계 3 (SPI 30MHz) | 개선 |
|-------------|---------------------|-------------------|------|
| 200KB | ~25초 | < 0.1초 | 250배+ |
| 500KB | ~60초 | < 0.2초 | 300배+ |
| 1MB | ~120초 | < 0.3초 | 400배+ |
...
**이 단계의 특징:**
- S3 OTA가 사실상 순간 완료 (1MB 이하 0.3초)
- 기존 SPI 카메라 링크의 프로토콜 확장 (새 type 추가만으로 가능)
- UART OTA(단계 2)는 fallback으로 유지
...
단계 2 (기능 안정화 후 — 양산 전 필수)
  └─ + Wi-Fi OTA
     ✅ RTL HTTP OTA (/ota)
     ✅ S3 UART OTA (/ota_s3, RTL 경유)
     ✅ (선택) DeepCoConnector OTA UI
     → 예상 구현: RTL ~300줄 + S3 ~200줄

단계 3 (최적화 — 필요 시)
  └─ + SPI 고속 OTA
     ✅ S3 OTA를 SPI로 전환 (UART fallback 유지)
     → 예상 구현: RTL ~100줄 + S3 ~100줄 (프로토콜 확장)
```
