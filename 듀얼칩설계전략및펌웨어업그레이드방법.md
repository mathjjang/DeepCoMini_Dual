...
> **OTA 파티션 참고:**
> - **RTL8720DN** — AmebaD SDK가 OTA1/OTA2 듀얼 파티션을 기본 플래시 레이아웃에 포함.
>   부트로더가 자동으로 유효한 파티션을 선택하여 부팅. **별도 설정 불필요.**
> - **ESP32-S3** — 기본 파티션 테이블은 단일 `factory` 앱 영역만 존재.
>   OTA를 사용하려면 `partitions.csv`를 수정하여 `ota_0` + `ota_1` 듀얼 앱 파티션으로 변경 필요.
>
> ```
